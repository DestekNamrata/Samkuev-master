...
import 'package:samku/src/requests/prebook_request.dart';
...
class OrderController extends GetxController with SingleGetTickerProviderMixin {
  ...
  Future<List> getOrderHistory(Users user, int status, int idLang) async {
    int limit = 10;
    int offset = 0;
    List orders = [];
    ordersList.value = [];
  ...
  //for prebook
  Future<List> getPreBook(Users user, int status, int idLang,int isTabFlag) async {
    int limit = 10;
    int offset = 0;
    List preBook = [];
    preBookList.value = [];
    preBookList.refresh();
    Map<String, dynamic> data =
    await preBookRequest(user.id!, status, idLang, limit, offset,isTabFlag);

    if (data['success']) {
      preBook = data['data'];
      preBookList.value = data['data'];
      load.value = false;
    }
